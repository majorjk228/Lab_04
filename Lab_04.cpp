...
void main()
{
	...
	if (size <= 100)			// Защита от дурака
	{
		...
		int first_chet;										// Переменная для  хранения индекса массива с первым четным элементом 
		...
		for (int i = size++; i > first_chet; --i)				// Присваиваю i инкрементное значение длины нового массива.  size++ необходим для добавление нового элемента, в который будет присвоин m[фч -1] +2															
		{														// Иду с конца массива, и перетягиваю с конца каждое значение
			numbers[i] = numbers[i - 1];						// Добавляю новый элемент массива после первого четного числа
		}
		 
		numbers[first_chet + 1] = numbers[first_chet - 1] + 2;  // Присваиваю значение в новый добавленный элемент массива
